...
batterytype = ['LiFePO4', 'NiMh', 'PbAcid-1', 'PbAcid-3']
motortype = ['base', 'base_he', 'torque', 'torque_he']
chassismaterial = ['steel', 'magnesium']
modulenum = np.array([3, 6, 9, 12])
...
for cmaterial in chassismaterial:
    edl_system = define_chassis(edl_system,cmaterial)
    for btype in batterytype:
        for nmodule in modulenum:
            if btype in ['PbAcid-1', 'PbAcid-3'] and nmodule > 9:
                print(f'Skipping heavy battery combo: {btype} with {int(nmodule)} modules')
                continue
            edl_system = define_batt_pack(edl_system,btype, nmodule)
            for mtype in motortype:
                edl_system = define_motor(edl_system,mtype)
                ...
                if feasible:
                    xbest = res.x
                    fbest = res.fun
                else:  # Skip this combination if not feasible
                    print(f'Solution not feasible for {cmaterial}, {btype}, {mtype}, {int(nmodule)} modules - skipping...')
                    continue
                ...
                run_counter += 1
                filename = f'run_{cmaterial}_{btype}_{mtype}_{int(nmodule)}.txt'
                
                with open(filename, 'w') as f:
                    f.write(f'Configuration:\n')
                    f.write(f'Chassis Material: {cmaterial}\n')
                    f.write(f'Battery Type: {btype}\n')
                    f.write(f'Number of Battery Modules: {int(nmodule)}\n')
                    f.write(f'Motor Type: {mtype}\n')
                    f.write(f'\n')
                    f.write(f'Results:\n')
                    f.write(f'Optimized parachute diameter   = {xbest[0]:.6f} [m]\n')
                    f.write(f'Optimized rocket fuel mass     = {xbest[4]:.6f} [kg]\n')
                    f.write(f'Time to complete EDL mission   = {time_edl:.6f} [s]\n')
                    f.write(f'Rover velocity at landing      = {edl_system["rover_touchdown_speed"]:.6f} [m/s]\n')
                    f.write(f'Optimized wheel radius         = {xbest[1]:.6f} [m]\n')
                    f.write(f'Optimized d2                   = {xbest[3]:.6f} [m]\n')
                    f.write(f'Optimized chassis mass         = {xbest[2]:.6f} [kg]\n')
                    f.write(f'Time to complete rover mission = {time_rover:.6f} [s]\n')
                    f.write(f'Total mission time             = {total_time:.6f} [s]\n')
                    f.write(f'Average velocity               = {edl_system["rover"]["telemetry"]["average_velocity"]:.6f} [m/s]\n')
                    f.write(f'Distance traveled              = {edl_system["rover"]["telemetry"]["distance_traveled"]:.6f} [m]\n')
                    f.write(f'Battery energy per meter       = {edl_system["rover"]["telemetry"]["energy_per_distance"]:.6f} [J/m]\n')
                    f.write(f'Total cost                     = {edl_system_total_cost:.6f} [$]\n')
                ...
                if total_time < best_time:
                    ...
                    # Save the best configuration with a special marker
                    best_filename = f'BEST_{run_counter}_run_{cmaterial}_{btype}_{mtype}_{int(nmodule)}.txt'
                    with open(best_filename, 'w') as f:
                        f.write(f'*** BEST CONFIGURATION SO FAR ***\n')
                        f.write(f'Run Number: {run_counter}\n')
                        f.write(f'Total Mission Time: {total_time:.6f} [s]\n')
                        f.write(f'\n')
                        f.write(f'Configuration:\n')
                        f.write(f'Chassis Material: {cmaterial}\n')
                    ...
                    print(f'*** NEW BEST TIME: {total_time:.6f} [s] - Saved to {best_filename} ***')
